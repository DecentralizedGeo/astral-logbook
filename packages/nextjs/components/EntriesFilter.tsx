...
const EntriesFilter: React.FC<EntriesFilterProps> = ({ onFilterChange, entriesCount, filteredCount }) => {
    ...
    <div className="bg-white rounded-lg shadow mb-4 overflow-hidden">
      ...
      >
        <div className="font-medium flex items-center">
          ...
          {hasActiveFilters() && (
            <span className="ml-2 bg-white text-primary text-xs px-2 py-1 rounded-full">Active</span>
          )}
        </div>
            ...
            <div className="flex space-x-2">
              <button
                className={`px-3 py-1 text-sm rounded-md ${
                  filters.hasMedia === true ? 'bg-primary text-primary-content' : 'bg-gray-200 text-gray-700'
                }`}
              ...
              <button
                className={`px-3 py-1 text-sm rounded-md ${
                  filters.hasMedia === false ? 'bg-primary text-primary-content' : 'bg-gray-200 text-gray-700'
                }`}
            ...
            <div className="flex flex-wrap gap-2">
              {[
                ...
                <button
                  key={time.key}
                  className={`px-3 py-1 text-sm rounded-md ${
                    filters.timeOfDay[time.key as keyof typeof filters.timeOfDay]
          ...
          <div className="flex justify-end">
            <button
              ...
              className={`px-4 py-2 text-sm rounded-md ${
                hasActiveFilters() ? 'bg-red-500 text-white' : 'bg-gray-200 text-gray-500'
              }`}
